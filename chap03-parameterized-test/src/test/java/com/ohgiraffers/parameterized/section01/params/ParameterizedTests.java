...
class ParameterizedTests {
    ...
    /*
    * 테스트 시 여러 값들을 검증을 해야 하는 모든 경우의 수만큼 테스트 메소드를 작성해야 한다.
    ...
    * */
    ...
    @ParameterizedTest //
    @ValueSource(ints = {1, 3, -1, 15, 123}) // 위의 것과 이 부분이 given의 역할을 대신한다. 이 값이 매개변수에 들어간다.
    ...
    * */
    ...
    @ParameterizedTest
    @NullSource // null 값도 적용될 매개변수가 필요하다.
    ...
    void testMontHValueIsCorrect(Month month){

        // when
        boolean result = DateValidator.isCorrect(month);

        // then
        Assertions.assertTrue(result);
    }

    @DisplayName("2월, 4월, 6월, 9월, 11월을 제외한 달이 31일지 확인")
    @ParameterizedTest
    @EnumSource(
            value = Month.class,
    ...
    void testHasThirtyOneDaysLong(Month month){ // 매개변수도 given으로 취급한다.
        // given
        int verifyValue = 31;
