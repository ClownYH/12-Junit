...
public class EnvAnnotationTests {
    ...
    /*
    * @EnabledOnJre를 이용하면 특정 JRE 버전에서만 테스트하는 것도 가능하다.
    * @DisabledOnJre를 이용하여 특정 JRE 버전에서만 테스트를 Disabled 하는 것도 가능하다.
    * @EnabledForJreRange를 이용하면 min과 max 속성 사이의 버전에서 테스트 하는 것도 가능하다.
    * (min만 작성 시 min~최신 버전까지, Max만 작성 시 이전 버전부터 max까지만 테스트한다.)
    * @DisableForJreRange를 이용하면 특정 범위 상이의 JRE 환경에서의 테스트를 disabled 할 수 있다.
    * */
