...
import com.ohgiraffers.assertions.section02.Member;
...
public class AssertJTests {
    ...
    * */
    ...
    // 날짜 객체가 가지는 년, 월, 일, 시, 분, 초. 밀리초, 나노초까지도 검증이 가능하며, 특정 날씨 범위, 이전, 이후 내용도 검증이 가능하다.
    @Test
    ...
    void testExceptionValidation(){
        ...
        // given
        // when
        Throwable thrown = AssertionsForClassTypes.catchThrowable(() -> {throw new IllegalArgumentException("잘못된 파라미터입니다.");});
    ...
    void testExceptionValidation2(){
        ...
        /* 필기
        *   자주 사용되는 예외 처리에 대한 정의된 함수를 제공한다.
...
//                }).withMessageContaining("파라미터");
    ...
    /* 목차. 6 filter를 이용한 단정문 테스트하기*/
    /* 필기. 특정 filter를 자바 람다식으로 */
    @Test
    ...
    void testFilteringAssertions(){
        ...
        Assertions.assertThat(members)
                .filteredOn(member -> member.getAge() > 20) // 리액트 map처럼 뿌림, 이것만 가지고는 검사가 되지 않는다.
    ...
    void testExtractProperty() {
        ...
        //then
        Assertions.assertThat(members)
                .extracting("name", String.class) // 적출하는 것
                .containsAll(expected); // 순서는 상관없음
    }
    ...
    void testExtractPropertyTuple() {
        ...
        Assertions.assertThat(members)
                .extracting("name", "age")
                .contains(
                        Tuple.tuple("홍길동", 20), // 셀 수 있는 수량의 순서있는 열거 또는 어떤 순서를 따르는 요소들의 모음을 튜플(tuple)이라고 합니다
                        Tuple.tuple("유관순", 16) // 순서의 영향을 받는다.
                );
    }
}
